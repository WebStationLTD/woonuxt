<template>
  <Popover v-slot="{ open, close }" class="relative isolate z-50">
    ...
    <PopoverPanel
      static
      v-slot="{ open: panelOpen }"
      class="mega-menu-panel transition-all duration-200 bg-gradient-to-br from-white via-gray-50 to-red-50/30"
      ...
      <div class="mx-auto max-w-7xl px-3 md:px-6 lg:px-8 py-4 md:py-8">
        ...
        <div v-else-if="mainCategories?.length">
          ...
          <div class="hidden md:grid md:grid-cols-3 gap-4 mb-6">
            <div v-for="mainCat in mainCategories" :key="mainCat.slug" class="group relative">
              <!-- Основна карта -->
              <NuxtLink
              ...
              <div
                class="absolute top-full left-0 right-0 mt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                <div class="bg-white rounded-xl shadow-2xl border border-gray-200 p-4 max-h-[400px] overflow-y-auto">
                  ...
                  <div :class="['gap-2', getSubcategories(mainCat.slug).length > 6 ? 'grid grid-cols-2' : 'space-y-2']">
                    <NuxtLink
          ...
          <div class="md:hidden space-y-2 mb-3">
            <div v-for="(mainCat, index) in mainCategories" :key="mainCat.slug" class="border border-gray-200 rounded-lg overflow-hidden">
              <!-- Header -->
              <div class="flex items-center bg-gray-50">
                <!-- Линк към категорията -->
                <NuxtLink :to="getCategoryUrl(mainCat)" @click="close" class="flex-1 min-w-0 px-3 py-2.5">
                ...
                <!-- Бутон за разгъване -->
                <button @click="toggleMobileCategory(index)" class="px-3 py-3 border-l border-gray-200 active:bg-gray-100 flex-shrink-0">
                  <Icon
              ...
              <!-- Подкатегории -->
              <div :class="['overflow-y-auto transition-all duration-300 bg-white', expandedMobileCategories[index] ? 'max-h-[300px]' : 'max-h-0']">
...
const mainCategories = computed(() => {
  ...
  const categories = categoryOrder.map((slug) => parents.find((cat) => cat.slug === slug)).filter(Boolean); // Премахваме undefined стойности
...
.mega-menu-panel {
  ...
  box-shadow:
    0 20px 25px -3px rgba(0, 0, 0, 0.1),
